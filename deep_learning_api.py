...
def get_signal(data_file):
    ...
    for line in data_file:
        try:
            patient_id, device_id, patient_name, bpm = line.strip().split("|")
            l.append(int(bpm))
        except:
...
def ml_prediction(data_file):
    ...
    try:
        predicted_status = int(stats.mode(result)[0])
    except:
        predicted_status = -1 # unknown
